<template>
  ...
  <UsersValidationList :utilisateurs="filteredUtilisateurs"/>
</template>

<script setup>
import { ref, computed } from 'vue';
import UsersValidationList from '../../../components/validations/enseignants/UsersValidationList.vue';
...
const filteredUtilisateurs = computed(() => {
  if ( filters.value.length === 0 ) return utilisateursStore.getUsersUnverified();
  return utilisateursStore.getUsersUnverified().filter((utilisateur) => {
